...
makeHM = function(myMat, OutputPath){
   ...
   heatmap.2(myMat, trace = "none", dendrogram = "none", Rowv = FALSE, Colv = FALSE, col = myPalate, cellnote = round(myMat,2), notecol = "black", margins = c(20,22), main = "Correlation By Tool and Cell Type", breaks = c(-1,-.15,0.0,.2,.4,.8,.9,1.0), cexRow = 3, cexCol = 3, notecex = 3)
   dev.off()
}
...
makeHM(CorrCellMix, "CorrsByRef/Fig1c.png")
makeHM(CorrPBMC, "CorrsByRef/Fig1a.png")
makeHM(CorrStromal, "CorrsByRef/Fig1b.png")
makeHM(CorrFram, "CorrsByRef/Fig1d.png")


#Code for generating separate plots for PBMC1 and PBMC2, not used in final paper

#DataPBMC1 = read.table("../3CombineAndUnroll/Unrolled_PBMC1.tsv", header = TRUE, sep = "\t")
#DataPBMC2 = read.table("../3CombineAndUnroll/Unrolled_PBMC2.tsv", header = TRUE, sep = "\t")
